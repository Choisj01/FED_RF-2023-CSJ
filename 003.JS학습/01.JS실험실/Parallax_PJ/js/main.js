...
/********************************************************* 
    [페럴렉스 기능구현하기]
    1. 정의 : 페럴렉스란 스크롤 작동시 같은 방향으로 요소가 
    다른속도를 가지고 움직임으로 사용자가 공간감을 
    느낄 수 있게 하는 구현방법
    2. 방법:
        (1)범위 -요소가 화면에 등장하여 보일동안 스크롤될때
        이동함
        (2)움직일 크기 설정이 필요함
        (3)범위체크를 위한 JS메서드를 사용함
...
function moveEl(elPos,ele,setH){
    // (1) 전달변수 : elPos - 위치값(getBCR값)
    // (2) ele - 대상요소(패럴렉스대상)
    // (3) setH - 움직일 범위 수(클수록 빠르게 이동)
    // console.log('위치:',elPos,'\n대상:',ele,'\n범위:',setH);

    // [패럴렉스 범위 : 윈도우 높이값~0]
    // 화면에서 완전히 사라질떄 범위는 0보다 작다(약간의 마이너스값)
