...
// 1. 요구사항 : 배열에 과일 정보를 담아서 "과일주세요"버튼 클릭시
...
// 2-2. 변경대상 : .showit (배열정보 출력) / .cont (과일출력박스)
const showit = dFn.qs(".showit");
const cont = dFn.qs(".cont");
// 2-3. 전체과일 콤보박스 : #sel
const sel = dFn.qs('#sel');
// 2-4. 선택과일 콤보박스(anum - array number) : #anum
...
function showFruit() {
    ...
    if (btxt == "과일주세요~!") {
        ...
        let hcode = `<ul>`;
        fruit.forEach((val) => {
            hcode += `<li style="background:url(./addimg/${frObj[val]}.png) no-repeat center/cover">${val}</li>`;
        }); /////////forEach//////////////
        hcode += `</ul>`;
    ...
    //          splice(순번,개수) -> 순번부터 개수만큼 삭제
        ...
        console.log('지울순번:',anum.value,'/지울갯수:',delNum.value);
