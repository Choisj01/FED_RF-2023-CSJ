// JS4-4. 배열의 정렬과 검색 JS - JS4-4.배열정렬및검색.html
...
//////////////////////////////////////////////
// 1. 숫자로만 된 배열의 화면 뿌리기///////////
// map메서드로 배열값을 태그로 감싸서 출력하기
// (1) 출력대상: .showNum
const showNum = dFn.qs(".showNum");
// (2) 배열 숫자 데이터만큼 이미지로 변환하여 출력
// map().join() 맵조인!
const showScreen = () =>
    (showNum.innerHTML = arrNumber.map((val) => `<img src="./images/num/num_0${val}.png" alt="숫자이미지">`).join(""));
...
// sel3 이벤트 설정하기
// 데이터와 출력 타겟부터 설정 후 정렬함수 호출!(등록순서)
...
dFn.addEvt(sel4, "change", () => {
    // 정렬용 데이터는 원본 list2 쓰지 않고 newList 사용!(바뀐 데이터 업데이트)
    targetData = newList;
    targetEle = showList4;
}); /////////change //////////
