...
export function SwiperApp() {
  ...
  const stopPlay = () => {
    ...
    console.log('리랜더링:',myFirst,mySecnod.current);
  ...
  let myFirst = '스와이퍼';
  const mySecnod = useRef('갤러리');

  //변수값 업데이트 함수
  const myFn = () => {
    myFirst = 'Swiper';
    mySecnod.current='Gallery';
    console.log('함수호출:',myFirst,mySecnod.current);
  }; /////////myFn함수//////////////
  ...
  return (
    <>
    <h1 style={{padding:'20px'}}>{myFirst+' : '+mySecnod.current}</h1>
      <Swiper
      ///* ref 속성에 useRef할당변수를 넣어서 외부에 연결함 */
      ref={myRef}
        slidesPerView={3}
        spaceBetween={30}
        pagination={{
