...
function EventShow() {
    // 1. 컴포넌트에 사용하는 내부용 함수작성
    // (1) 소원이 무엇이냐 실행 함수
    const aladin = lamp => {
        //lamp - 알라딘 주인고 이미지경로
        console.log("aladin함수:", lamp);
        // (1-1) #tbox 인 요소의 내부에 h1 요소 넣기
        ...
        //  (1-4) 랩프 가져오기 버튼 3초 후 보이기
        setTimeout(() => {
            document.querySelectorAll('button')[0]
            .style.display = 'inline-block';
        }, 3000);
        // (1-5) #ala 요소에 이미지 출력하기
        // ReactDOM.render(어쩌고,저쩌고)
        ReactDOM.render(
...
function MakeImg(props){ // isrc - 이미지경로 / ialt - 이미지설명
    ...
    // true 이면 페라리용 이미지 태그로 생성함!
    console.log('구분속성 props.info:',props.info);
    return(props.info?
        <img id={props.idnm} src={props.isrc} alt={props.ialt} title={props.tit} onClick={moveCar} />:
