...
function MainComponent() {
    // 상태관리 메서드를 사용하여 후크(Hook)변수를 설정하자!
    // const [변수명,set변수명] = React.useState(초기값)
    // dataNum은 데이터를 구분하는 번호저장 후크변수다!
    // 데이터 구분값으로 배열순번을 생각하여
    // 처음에 로딩될 데이터가 0번째 즉,
    // 첫번째 배열순번 데이터를 불러올 순번값을 셋팅함!
    ...
    //  가상돔에서 실제돔에 반영 후 DOM에
    // 구현할 JS코드는 어디에 넣어야 하는가?
    // ->>>>>useEffect()
    // ->>>>>useLayoutEffect()
    ...
    React.useEffect(() => {
        ...
        // console.log("useEffect 순수구역 제이쿼리:",
        // $(".img-box"));
    });
    ...
    React.useLayoutEffect(() => {
        console.log("useLayoutEffect 구역JS:")
        // 이미지를 display:none
        // $(".btn-gong").hide();
    });
...
// 서브 컴포넌트(자식 컴포넌트-> 부모컴포넌트로부터 데이터를 props속성을 통하여 전달받는다!)
// 상품 html 코드 구성 컴포넌트//////////////////////
