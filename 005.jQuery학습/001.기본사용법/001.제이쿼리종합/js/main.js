...
btns.first() // 첫번째 버튼
    ...
    // 6. "윗층으로 도망가!" 버튼 클릭시 /////// 위의 버튼에서 이어짐!(이어지게 하기 위해서 들어가기버튼 끝에 ;(세미콜론) 생략!)
    .next() // 세번째버튼
    .click(function () {
        // 하위 이벤트함수 this의미!
        // ()=>{
    ...
    // 7. "다시옆방으로!" 버튼 클릭시 /////// 위의 버튼에서 이어짐!(이어지게 하기 위해서 들어가기버튼 끝에 ;(세미콜론) 생략!)
    .next() // 네번째버튼
    .click(function () {
        // 하위 이벤트함수 this의미!
        // ()=>{
    ...
    // 7. "무서우니 윗층으로!" 버튼 클릭시 /////// 위의 버튼에서 이어짐!(이어지게 하기 위해서 들어가기버튼 끝에 ;(세미콜론) 생략!)
    .next() // 다섯번째버튼
    .click(function () {
        // 하위 이벤트함수 this의미!
        // ()=>{
        ...
        let fn =
            // function(){ -> this가 mi임!
            () => {
              //무.서.워...메시지 -배배배(배열안에 배열안에 배열!)   
              msg
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][1]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][2]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][3]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][4]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][5]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][6]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,()=>msg.html(msgTxt[4][0][7]))
              .html(msgTxt[4][0][0]).fadeIn(200).delay(500).fadeIn(200,
                ()=>{ //무서워 대사 후 좀비 올라와 달려들기!!
                    ...
                    },500,'easeInOutElastic') ///animate/////
                    .delay(500) // 기다림
                    .animate({
                        // right값을 li width 값만큼 이동 (120%보정)
