...
$('.ham').click(()=>{
    ...
    // is() 메서드 : 선택요소의 이름 확인!
    console.log('지금.header에 .on있니?',$('.header'),hEle.is('.on'));
...
$('.gnb li').click(function(){
    ...
    .parent() //부모로 올라감!
    .siblings().find('.smenu') //다른 li하위 .smenu
    .slideUp(300,'easeInOutQuad'); //스르륵 닫힘! 모두

}); ///////////click///////////

// 3. sticky 메뉴박스 드래그하여 움직여보기
// 대상 :dokebi-menu ul
$('.dokebi-menu ul')
.draggable({
    axis:'x', //x 축 고정
...
// 대상:intro-mv-img
// 이벤트: click
// -> 가상요소 플레이버튼 클릭시 이벤트 버블링으로 본박스가 반응함!
...
// 만든어진 새로운 배열을 변수엔 담거나 그자리에 리턴한다!
...
subViewBox.click(function () {
    console.log("나야나!", this); //this - 클릭된 요소 자신
