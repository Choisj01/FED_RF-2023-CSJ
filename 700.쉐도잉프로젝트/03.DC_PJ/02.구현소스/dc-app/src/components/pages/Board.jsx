  ...
  *************************************/
    const chgMode = (e) => {
        ...
        if (modeTxt === "R") {
            ...
            // 로그인 사용자와 글쓴이가 같으면 btnSts 상태값 true
    ...
    // 사용자 비교함수 /////////////
    // 원본으로부터 해당 사용자정보 조회하여
    // 글쓴이와 로그인 사용자가 같으면 btnSts값을 true로 업데이트
    const compUsr = (usr) => {
        // usr - 글쓴이 아이디(uid)
        // opt - 리턴받을 데이터종류
        // unm - 사용자 이름 , eml - 사용자 이메일
    ...
    조건 : 
        ...
        업데이트를 막아준다!
        (이때 배열은 세션스에 기록함! 이유는 브라우저
        닫을 때 사라짐!)

        * 업데이트 시점 : 글읽기 모드에 들어간 후 
  ********************************************************/
    const plusCnt = () => {
        // 0. 처음에 통과상태 설정하기
        let isOK = true;
        // 세션스에 등록된 글 or 로그인 사용자글일떄 false처리!

        // 1. [ 현재읽은 글은 cData.current로 읽어옴!]
        let cidx = cData.current.idx;
        // console.log("조회수 증가 체크 idx:", cidx);

        // 2. [ 세션스에 등록된 글idx가 있는지 여부 확인하기 ]
        ...
        // isOK값을 false로 처리함!(some메서드는 돌아주다가 있으면 거기서 멈추고 더이상 돌리지 않음!)
        ...
        if (isOK) {
            ...
            // 원본데이터에 반영하기 : 꼭 해야만 리스트가 업데이트됨!
            orgData = data;

            // 반영된 배열데이터를 다시 'bdata' 로컬스에 넣기
    ...
    const searchList = () => {
        ...
        const resData = storageData.filter((v) => {
            ...
            // 대괄호로 객체값을 읽어온다!
            // indexOf() 로 like 검색함!
            if (v[cta].indexOf(inpVal) !== -1) return true;
        });
